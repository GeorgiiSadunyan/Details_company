...
class Supplier_rep_DB:
    ...
    def _execute_update(self, query: str, params: tuple = ()):
        """Вспомогательный метод для UPDATE, INSERT, DELETE"""
        with self.conn.cursor() as cur:
            cur.execute(query, params)
    ...
    def get_k_n_short_list(self, k: int, n: int) -> list[SupplierMini]:
        ...
        LIMIT %s OFFSET %s;
        """
        result = self._execute_query(query, (n, offset))
    ...
    def add(self, supplier: Supplier): 
        ...
        result = self._execute_query(query, (supplier.name, supplier.phone))
        if result:
    ...
    def replace_by_id(self, supplier_id: int, supplier: Supplier):
        ...
        WHERE supplier_id = %s;
        """
        self._execute_update(query, (supplier.name, supplier.phone, supplier.address, supplier_id))

    # e. Удалить элемент списка по ID
    def remove_by_id(self, supplier_id: int):
        query = "DELETE FROM suppliers WHERE supplier_id = %s;"
        self._execute_update(query, (supplier_id,))
